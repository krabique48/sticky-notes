...
class User < ActiveRecord::Base
  class << self
    def from_omniauth(auth_hash)
      ...
      user.image_url = auth_hash['info']['image'].blank? ? "/user-200.png" : auth_hash['info']['image']
    ...
    def get_social_location_for(provider, location_hash)
      case provider
        when 'linkedin'
          ...
          location_hash.blank? ? "-" : location_hash
      end
    end
    ...
    def get_social_url_for(provider, urls_hash)
      case provider
        when 'linkedin'
          ...
          urls_hash.blank? ? "none" : urls_hash[provider.capitalize]
      end
    end
  end
